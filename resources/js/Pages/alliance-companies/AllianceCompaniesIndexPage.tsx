...
export default function AllianceCompaniesIndexPage(): React.JSX.Element {
    const [filters, setFilters] = useRemember<AllianceCompanyFilters>({ page: 1, perPage: 15 }, 'alliance-companies-filters');
    ...
    const { deleteAllianceCompany } = useAllianceCompanyMutations();
    ...
    const handleExport = (format: 'excel' | 'pdf') => {
        startExportTransition(() => {
            ...
            window.open(`/alliance-companies/data/export?${params.toString()}`, '_blank');
        });
    };
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <p className="text-sm mt-1 text-(--text-muted) font-medium">
                                Manage your Alliance partners — <span className="text-(--accent-primary)">{meta.total} {meta.total === 1 ? 'partner' : 'partners'}</span> found
                            </p>
                        </div>
                        <Link
                        ...
                        {meta.lastPage > 1 && (
                            <div className="flex items-center justify-between px-6 py-4 border-t border-(--border-subtle) bg-white/5">
                                <span className="text-xs font-semibold text-(--text-disabled) uppercase tracking-wider">
                                    Page {meta.currentPage} / {meta.lastPage} â€¢ {meta.total} Total
                                </span>
