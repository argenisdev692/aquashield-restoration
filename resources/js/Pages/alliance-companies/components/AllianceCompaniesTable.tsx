...
export default function AllianceCompaniesTable({
...
}: AllianceCompaniesTableProps) {
    const columnHelper = createColumnHelper<AllianceCompany>();
    const { restoreAllianceCompany } = useAllianceCompanyMutations();

    const columns = React.useMemo<ColumnDef<AllianceCompany, any>[]>(() => [
        ...
        columnHelper.accessor('alliance_company_name', {
            header: 'Company Name',
            ...
            cell: (info) => {
                ...
                return (
                    <div className="flex items-center justify-center gap-1.5">
                        <Link 
                            href={`/alliance-companies/${company.uuid}`}
                        ...
                        {!isDeleted ? (
                            <>
                                ...
                                <button 
                                    onClick={() => onDelete(company.uuid, company.alliance_company_name)}
                            ...
                            <button 
                                onClick={() => restoreAllianceCompany.mutate(company.uuid)}
                                className="btn-action btn-action-restore"
                                title="Restore Carrier"
                                disabled={restoreAllianceCompany.isPending}
                            >
                                <RotateCcw size={14} />
                            </button>
                        )}
    ...
    ], [columnHelper, onDelete, restoreAllianceCompany]);
