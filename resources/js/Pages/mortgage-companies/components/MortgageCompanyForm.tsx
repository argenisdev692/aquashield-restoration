...
interface MortgageCompanyFormProps {
    ...
    onSubmit: (data: MortgageCompanyFormData) => Promise<void> | void;
    isSubmitting: boolean;
    onCancel: () => void;
}

interface MortgageCompanyFormState {
...
export default function MortgageCompanyForm({
    ...
    isSubmitting,
    onCancel,
}: MortgageCompanyFormProps): React.JSX.Element {
    const [formData, setFormData] = React.useState<MortgageCompanyFormState>({
        mortgage_company_name: initialData?.mortgage_company_name ?? '',
        address: initialData?.address ?? '',
        phone: initialData?.phone ?? '',
        email: initialData?.email ?? '',
        website: initialData?.website ?? '',
    });
    const [address2, setAddress2] = React.useState<string>(initialData?.address_2 ?? '');
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>): Promise<void> {
        ...
        const companyName = formData.mortgage_company_name.trim();

        if (companyName === '') {
            setNameError('Mortgage company name is required.');
            return;
        }
        ...
        await onSubmit({
            mortgage_company_name: companyName,
            address: normalizeOptional(formData.address),
            address_2: normalizeOptional(address2),
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8 p-8">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <PremiumField
                    ...
                    required
                    value={formData.mortgage_company_name}
                    error={nameError}
                    onChange={(event) => {
                        setFormData((current) => ({
                            ...current,
                            mortgage_company_name: event.target.value,
                        }));
                        if (nameError !== '') {
                            setNameError('');
                        }
            ...
            >
                ...
                >
                    ...
                    <span>{isSubmitting ? 'Saving...' : initialData ? 'Save changes' : 'Create mortgage company'}</span>
                </button>
            </div>
        </form>
    );
