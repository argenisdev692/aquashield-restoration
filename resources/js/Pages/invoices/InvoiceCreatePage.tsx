...
import { useCreateInvoice } from '@/modules/invoices/hooks/useInvoiceMutations';
...
export default function InvoiceCreatePage(): React.JSX.Element {
            ...
            <AppLayout>
                <div className="flex flex-col gap-6" style={{ maxWidth: 900, margin: '0 auto' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <Link href="/invoices" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: 'var(--radius-md)', border: '1px solid var(--border-default)', background: 'var(--bg-card)', color: 'var(--text-muted)', textDecoration: 'none' }} aria-label="Back">
                            <ArrowLeft size={16} />
                        </Link>
                        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: 'var(--text-primary)', fontFamily: 'var(--font-sans)' }}>New Invoice</h1>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                        ...
                        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)', padding: 24 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                                <div>
                                    ...
                                    <input id="invoice_number" type="text" required value={form.invoice_number} onChange={(e) => handleChange('invoice_number', e.target.value)} style={inputSx} placeholder="INV-0001" />
                                </div>
                                <div>
                        ...
                        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)', padding: 24 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                                <div>
                                    ...
                                    <input id="bill_to_name" type="text" required value={form.bill_to_name} onChange={(e) => handleChange('bill_to_name', e.target.value)} style={inputSx} placeholder="Full name or company" />
                                </div>
                                <div>
                                    <label htmlFor="bill_to_email" style={labelSx}>Email</label>
                                    <input id="bill_to_email" type="email" value={form.bill_to_email} onChange={(e) => handleChange('bill_to_email', e.target.value)} style={inputSx} placeholder="billing@example.com" />
                                </div>
                                <div>
                                    <label htmlFor="bill_to_phone" style={labelSx}>Phone</label>
                                    <input id="bill_to_phone" type="text" value={form.bill_to_phone} onChange={(e) => handleChange('bill_to_phone', e.target.value)} style={inputSx} placeholder="+1 (555) 000-0000" />
                                </div>
                                <div>
                                    <label htmlFor="bill_to_address" style={labelSx}>Address</label>
                                    <input id="bill_to_address" type="text" value={form.bill_to_address} onChange={(e) => handleChange('bill_to_address', e.target.value)} style={inputSx} placeholder="123 Main St, City, ST" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-lg)', padding: 24 }}>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                                <div>
                                    ...
                                    <input id="claim_id" type="number" value={form.claim_id} onChange={(e) => handleChange('claim_id', e.target.value)} style={inputSx} placeholder="Numeric claim ID" />
                                </div>
                                <div>
                                    <label htmlFor="claim_number" style={labelSx}>Claim #</label>
                                    <input id="claim_number" type="text" value={form.claim_number} onChange={(e) => handleChange('claim_number', e.target.value)} style={inputSx} placeholder="CLM-2026-001" />
                                </div>
                                <div>
                                    <label htmlFor="policy_number" style={labelSx}>Policy #</label>
                                    <input id="policy_number" type="text" value={form.policy_number} onChange={(e) => handleChange('policy_number', e.target.value)} style={inputSx} placeholder="POL-00000" />
                                </div>
                                <div>
                                    <label htmlFor="insurance_company" style={labelSx}>Insurance Company</label>
                                    <input id="insurance_company" type="text" value={form.insurance_company} onChange={(e) => handleChange('insurance_company', e.target.value)} style={inputSx} placeholder="Company name" />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <input id="type_of_loss" type="text" value={form.type_of_loss} onChange={(e) => handleChange('type_of_loss', e.target.value)} style={inputSx} placeholder="e.g. Water Damage" />
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <input id="price_list_code" type="text" value={form.price_list_code} onChange={(e) => handleChange('price_list_code', e.target.value)} style={inputSx} placeholder="e.g. FLM24" />
                                </div>
                            </div>
                            <div style={{ marginTop: 16 }}>
                                <label htmlFor="notes" style={labelSx}>Notes</label>
                                <textarea id="notes" value={form.notes} onChange={(e) => handleChange('notes', e.target.value)} rows={3} style={{ ...inputSx, height: 'auto', padding: 12, resize: 'vertical' }} placeholder="Additional notes…" />
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
                            <Link href="/invoices" style={{ display: 'inline-flex', alignItems: 'center', padding: '10px 20px', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-default)', background: 'var(--bg-card)', color: 'var(--text-primary)', fontSize: 13, fontWeight: 600, fontFamily: 'var(--font-sans)', textDecoration: 'none' }}>
                                Cancel
                            </Link>
                            <button type="submit" disabled={createInvoice.isPending} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 24px', borderRadius: 'var(--radius-md)', background: 'var(--accent-primary)', color: '#fff', fontSize: 13, fontWeight: 700, fontFamily: 'var(--font-sans)', cursor: createInvoice.isPending ? 'not-allowed' : 'pointer', opacity: createInvoice.isPending ? 0.7 : 1, border: 'none' }}>
