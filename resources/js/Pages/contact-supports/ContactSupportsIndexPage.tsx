...
import {
    ...
    useRestoreContactSupport,
} from "@/modules/contact-supports/hooks/useContactSupportMutations";
import { useContactSupports } from "@/modules/contact-supports/hooks/useContactSupports";
import type { ContactSupportFilters } from "@/modules/contact-supports/types";
import AppLayout from "@/pages/layouts/AppLayout";
import ContactSupportsTable from "./components/ContactSupportsTable";

export default function ContactSupportsIndexPage(): React.JSX.Element {
    const [filters, setFilters] = useRemember<ContactSupportFilters>({
        page: 1,
        per_page: 15,
    }, "contact-supports-filters");
    ...
    const restoreContactSupport = useRestoreContactSupport();
    const bulkDeleteContactSupports = useBulkDeleteContactSupports();

    const contactSupports = data?.data ?? [];
    ...
    async function handleConfirmDelete(): Promise<void> {
        ...
        await deleteContactSupport.mutateAsync(pendingDelete.uuid);
    ...
    async function handleConfirmRestore(): Promise<void> {
        ...
        await restoreContactSupport.mutateAsync(pendingRestore.uuid);
            ...
            <AppLayout>
                <div className="flex flex-col gap-6">
                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <Link href="/contact-supports/create" className="btn-primary inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold">
                            <Plus size={16} />
                            <span>New contact support</span>
                        </Link>
                    </div>
                    ...
                    <CrudFilterBar
                        ...
                        searchPlaceholder="Search support contacts..."
                        searchAriaLabel="Search support contacts"
                        ...
                        selects={[
                            {
                                ...
                                onChange: (value) => {
                                    startTransition(() => {
                                        setFilters((current) => ({
                                            ...current,
                                            read_state: value === "" ? undefined : value as "read" | "unread",
                                            page: 1,
                                        }));
                                    });
                                },
                                options: [
                                    { value: "", label: "All Read Status" },
                    ...
                    <DataTableBulkActions count={selectedCount} onDelete={handleBulkDelete} isDeleting={bulkDeleteContactSupports.isPending} />

                    <div className="card overflow-hidden p-0">
                        <ContactSupportsTable
                            data={contactSupports}
                            isPending={isPending}
                ...
                <DeleteConfirmModal
                    ...
                    isDeleting={deleteContactSupport.isPending}
                />

                <RestoreConfirmModal
                    isOpen={pendingRestore !== null}
                    entityLabel="contact support"
                    entityName={pendingRestore?.name}
                    onConfirm={() => {
                        void handleConfirmRestore();
                    }}
                    onCancel={() => setPendingRestore(null)}
                    isPending={restoreContactSupport.isPending}
                />
            </AppLayout>
        </>
    );
