...
<body>
    ...
    <table>
        ...
        <tbody>
            @forelse ($rows as $row)
                @php $isDeleted = isset($row->deleted_at) && $row->deleted_at !== null && $row->deleted_at !== '—'; @endphp
                <tr>
                    <td>{{ $row->claim_status_name ?? '—' }}</td>
                    <td>
                        @if (isset($row->background_color) && $row->background_color !== '—')
                            <span class="color-swatch" style="background-color: {{ $row->background_color }};"></span>
            ...
            @empty
                <tr><td colspan="5" style="text-align:center; color:#6b7280;">No records found.</td></tr>
            @endforelse
        </tbody>
    </table>
