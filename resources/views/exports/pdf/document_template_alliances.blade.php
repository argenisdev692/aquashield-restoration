...
<body>
    ...
    <table>
        ...
        <tbody>
            @forelse ($rows as $row)
                <tr>
                    <td>{{ $row['template_name_alliance'] }}</td>
                    <td>{{ $row['template_description_alliance'] }}</td>
                    <td>
                        <span class="badge badge-type">{{ $row['template_type_alliance'] }}</span>
                    </td>
                    <td>{{ $row['alliance_company_name'] }}</td>
                    <td>{{ $row['uploaded_by_name'] }}</td>
                    <td>{{ $row['created_at'] }}</td>
                </tr>
            @empty
                <tr>
                    <td colspan="6" class="empty-state">No document template alliances found.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
