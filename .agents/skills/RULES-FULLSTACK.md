...
- **Counter Text**: `{count} {count === 1 ? 'record' : 'records'} found` (singular/plural handling mandatory).
- **Status Filter**: Dropdown with "All Status", "Active", "Deleted" options.
- **Bulk Delete**: Complete pattern with `rowSelection` state and `DataTableBulkActions` component integration.
- **CSRF Protection**: Bulk actions must use the Inertia router (`router.get`, `router.post`).
- Search filter.
...
- [ ] Bulk Delete implemented via `rowSelection` + `DataTableBulkActions`
