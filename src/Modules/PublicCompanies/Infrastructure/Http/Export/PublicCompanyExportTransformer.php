...
final class PublicCompanyExportTransformer
{
    #[\NoDiscard]
    public static function transformForExcel(PublicCompanyEloquentModel $company): array
    {
        return $company
            |> self::extractPayload(...)
            |> self::formatDates(...)
    ...
    #[\NoDiscard]
    public static function transformForPdf(PublicCompanyEloquentModel $company): array
    {
        return $company
            |> self::extractPayload(...)
            |> self::formatDates(...)
    ...
    private static function extractPayload(PublicCompanyEloquentModel $company): array
    {
        return [
            'public_company_name' => $company->public_company_name,
            'email' => $company->email,
            'phone' => PhoneHelper::format($company->phone),
            'address' => trim(implode(', ', array_filter([$company->address, $company->unit ?: $company->address_2]))) ?: null,
    ...
    {
        foreach (['public_company_name', 'email', 'phone', 'address', 'website'] as $field) {
