...
Route::middleware(['permission:READ_INSURANCE_COMPANY'])->group(function (): void {
    Route::get('/insurance-companies', [InsuranceCompanyPageController::class, 'index'])->name('insurance-companies.index');
    Route::get('/insurance-companies/{uuid}', [InsuranceCompanyPageController::class, 'show'])->name('insurance-companies.show')->whereUuid('uuid');
});

Route::middleware(['permission:CREATE_INSURANCE_COMPANY'])->group(function (): void {
    Route::get('/insurance-companies/create', [InsuranceCompanyPageController::class, 'create'])->name('insurance-companies.create');
});

Route::middleware(['permission:UPDATE_INSURANCE_COMPANY'])->group(function (): void {
    Route::get('/insurance-companies/{uuid}/edit', [InsuranceCompanyPageController::class, 'edit'])->name('insurance-companies.edit')->whereUuid('uuid');
});
